    ...
    def go(self, doc, summ, measure_t=False, measure_summ=False):
        ...
        if measure_t:
            ...
            if self.return_token_lls:
                ...
                '''for simple
                results = dict()
                results['ll_base'] = ll_base
                results['ll_help'] = ll_help
                results['ll_full'] = ll_full
                results['S'] = S
...
if __name__ == '__main__':
    ...
    if args.simple:
        ...
        summ = 'Varvara the gray whale traveled from Russia to Mexico, a swim of record breaking length.'
        print(doc)
        print(summ)
        results = s.go(doc, summ, measure_t=args.measure_t, measure_summ=args.measure_summ)
        InfoDiff = results['ll_help']-results['ll_base']

        print('results',results)
        print('info diff',InfoDiff)
    else:
          ...
          for record in tqdm(selection):
            ...
            if args.measure_t or args.measure_summ:
                ...
                ll_base, ll_help, ll_full, S, num_doc_tokens, num_summ_tokens = s.go(
                    ' '.join(word_tokenize(record['text'])[:300]), record['decoded']
                )
                '''
                print(json.dumps({
                    'doc_id': record['id'],
